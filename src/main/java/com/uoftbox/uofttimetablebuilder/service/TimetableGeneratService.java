...
public class TimetableGeneratService {
    ...
    private void generateTimetables(CourseInfo[][] timetable, Queue<TimetableWithScore> queue, TimetableMetrics metrics, Map<String, Map<String, Map<String, List<CourseInfo>>>> courses, List<CourseInfo> currentTimetable, UserPreferences userPreferences, int courseIndex, int typeIndex, long timeStart) {
        ...
        if (typeIndex == courseTypes.size()) {
            generateTimetables(timetable, queue, metrics, courses, currentTimetable, userPreferences, courseIndex + 1, 0, timeStart);
            return;
        }
        ...
        for (Map.Entry<String, List<CourseInfo>> entry : sections.entrySet()) {
            ...
            boolean conflict = sectionInfo.stream().anyMatch(classInfo -> isConflict(classInfo, timetable));
            ...
            if (!conflict) {
                ...
                generateTimetables(timetable_copy, queue, metrics, courses, currentTimetable, userPreferences, courseIndex, typeIndex + 1, timeStart);
