...
fetch("./lib/test.json").then(function (response) {
    ...
    for (let product of products) {
        ...
        let location = product.time.location;
        let info = `Course: ${product.course}<br>Section: ${product.section}<br>Location: ${(location != "" ? location : "TBA")}`;
        timetable[timeStart][day] = `<td id="${info}" rowspan="${counter}" onclick="tdHaveSection(this)">${product.course}<br>${product.section}</td>`;
    }
